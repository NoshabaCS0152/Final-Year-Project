...
const RequestedAppointments = ({ route }) => {
    const data = route?.params?.data || [];
    const appointments = useContext(AppointmentsContext);
    const AppointmentRequests = appointments.filter(app => app.status === 'Appointment requested');


    console.log("dgggghdfthis is data of request appointments", AppointmentRequests)

    return (
        <View style={styles.container}>
            {AppointmentRequests.length === 0 ? (
                <Text style={styles.noAppointmentsText}>No appointment requests.</Text>
            ) : (
                <FlatList
                    data={AppointmentRequests} // <-- Use acceptedAppointments here
